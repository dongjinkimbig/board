...
<%
	...
	try {
		...
		if(conn != null){
			...
			String sql = "select vw_idx from tb_view where vw_boardidx=? and vw_userid=?";
			...
			sql = "insert into tb_view (vw_userid, vw_boardidx) values (?, ?)";
			pstmt = conn.prepareStatement(sql);
...
<script>
	...
	function like(){
		...
		if(isHeart){
			document.getElementById('heart').setAttribute('src', './unlike.png');
			document.getElementById('heart').setAttribute('title', 'off');
		}else{
			document.getElementById('heart').setAttribute('src', './like.png');
...
<body>
	...
	<table>
		...
		<tr>
			<th>좋아요</th><td> <%if(isLike){%>
			<img id="heart" src="./like.png" alt="좋아요" onclick="like()">
			<%}else{%>
			<img id="heart" src="./unlike.png" alt="좋아요" onclick="like()"><%}%> 
